...
/* 회원인데 가입 권유가 뜨는 것을 가리는 진단판입니다 (2026-09-25).
   ...
   ── 왜 주소에 ?진단=1 로 켜나 · 왜 관리자만인가 ──────────────
   처음 만들 때(2026-09-25 낮)는 **누구나** 켤 수 있게 했습니다.
   「관리자인지 아는 방법이 바로 그때 고장난 그 판정」이었기 때문입니다.
   그 고장은 고쳤으므로, 이제 관리자에게만 보입니다.

   다만 「모름」일 때는 보여줍니다. 관리자인지 **알 수 없는 상태 자체가**
   진단해야 할 고장이고, 그때 감추면 진단판이 있으나 마나입니다.
   모름일 때 나오는 것은 쿠키 개수·브라우저·배포번호뿐입니다 —
   값도, 토큰도, 사람 id 도 안 찍습니다.
...
/* 주소에 ?진단= 이 있고 **관리자이거나 아직 모를 때만** 켭니다 (2026-09-25) */
export function useDiag(): boolean {
  const { loading, isAdmin, who } = useAuth();
  const 주소에켜짐 = use진단주소();
  return 주소에켜짐 && (isAdmin || loading || who === '모름');
}

function use진단주소(): boolean {
