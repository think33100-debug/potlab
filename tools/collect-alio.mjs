...
async function 한건(cfg, r, 셈, 옵션) {
  ...
  let 근거 = job ? '제목' : '';
  const hay = Object.values(r).map((v) => String(v ?? '')).join(' ');
  ...
  if (!job && !옵션.상세안열기) {
    ...
    if (!box) { 셈.상세못받음++; hold = '상세를 못 받았습니다'; }
      ...
      if (!job) {
        const j2 = matchJob(String(box.aplyQlfcCn || '') + ' ' + String(box.prefCn || ''));
        if (j2) { job = j2; 근거 = '상세 자격'; }
      }
      /* ⑤ 첨부 공고문 (OCR) */
      if (!job) {
      ...
      if (!job && titleOtherOnly(title) && 근거 !== '상세 전형단계') { 셈.남의자리++; return null; }
      ...
      if (!job) {
        if (!MEDTECH.test(hay)) { 셈.우리와무관++; return null; }
        hold = 못읽은까닭 || '직군을 뭉뚱그린 공고입니다';
      }
